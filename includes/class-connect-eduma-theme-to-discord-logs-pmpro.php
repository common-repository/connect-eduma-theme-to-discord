 ...
 */
class Connect_Eduma_Theme_To_Discord_Logs_PMPRO {
	function __construct() {
		// Clear all existing logs.
		add_action( 'wp_ajax_ets_eduma_pmpro_discord_clear_logs', array( $this, 'ets_eduma_pmpro_discord_clear_logs' ) );
	}
	 ...
	 * @return None
	 */
	public function ets_eduma_pmpro_discord_clear_logs() {
		if ( ! is_user_logged_in() && ! current_user_can( 'administrator' ) ) {
			wp_send_json_error( 'You do not have sufficient rights', 403 );
			exit();
		}
		// Check for nonce security
		if ( ! wp_verify_nonce( $_POST['ets_eduma_pmpro_discord_nonce'], 'ets-eduma-discord-ajax-nonce' ) ) {
			wp_send_json_error( 'You do not have sufficient rights', 403 );
			exit();
		}
		try {
			$uuid      = get_option( 'ets_eduma_pmpro_discord_uuid_file_name' );
	...
	static function write_api_response_logs( $response_arr, $user_id, $backtrace_arr = array() ) {
		...
		$uuid             = sanitize_text_field( trim( get_option( 'ets_eduma_pmpro_discord_uuid_file_name' ) ) );
...
new Connect_Eduma_Theme_To_Discord_Logs_PMPRO();
